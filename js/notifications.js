...
const Notif = (() => {
  ...
  function _playChime(){
    ...
    // Ascending 4-note chime (notification/info)
  ...
  function _playMedAlert(){
    // Two-tone "medicine" sound — mellow double-ping
    const c=_ctx();if(!c)return;const now=c.currentTime;
  ...
  function _playMessageAlert(){
    // Short swoosh-up — chat message feel
    const c=_ctx();if(!c)return;const now=c.currentTime;
    const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);
  ...
  function _playSiren(){
    ...
    function burst(){
      if(_sirenCount>=12)return;_sirenCount++;
      const c=_ctx();if(!c)return;const now=c.currentTime;
      // Main siren sweep
      const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);
  ...
  function initPanel(uid,color){
    ...
    _panel.innerHTML='<div style="background:'+color+';color:#fff;padding:14px 18px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0">'
      ...
      +'<div style="display:flex;gap:8px;align-items:center">'
        +'<button onclick="Notif.markAllRead(\''+uid+'\');Notif.refreshPanel()" style="font-size:.67rem;background:rgba(255,255,255,.2);border:none;color:#fff;padding:3px 8px;border-radius:6px;cursor:pointer">Mark all read</button>'
  ...
  function refreshPanel(){
    ...
    const notifs=forUser(_uid).slice(0,80).reverse();
  ...
  function startListening(uid,onNew){
    _uid=uid;let seen=forUser(uid).length;
    window.addEventListener('storage',e=>{
      if(e.key!==KEY)return;const cur=forUser(uid).length;
      if(cur>seen){forUser(uid).slice(-(cur-seen)).forEach(n=>{showPopup(n.type,n.title,n.body,n.type==='sos_alert'?0:7000);playSound(n.type);_os(n.title,n.body);if(onNew)onNew(n);});seen=cur;}
  ...
  function startWater(mins,patientId,userId,caregiverId){
    ...
    _wId=setInterval(()=>{
      ...
      Storage.addHistory(patientId,'water','Water reminder every '+mins+' min');
      if(caregiverId){const u=Storage.userById(userId);push(caregiverId,'water_started','💧 '+(u?u.name:'Patient')+' water reminder fired','Hydration alert every '+mins+' min');}
    },mins*60*1000);
  }
    ...
    _mId=setInterval(()=>{
      ...
      (getFn()||[]).forEach(m=>{
        ...
        if(m.time===hhmm&&!_notifiedMeds.has(k)){
          ...
          Storage.addHistory(patientId,'medicine','Alert: '+m.name+' ('+m.dosage+') at '+hhmm);
